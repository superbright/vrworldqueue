...
describe('Bay', function () {
    ...
    it('should enqueue user', function () {
        return agent
            ...
            .then(function (res) {
                assert(res.body.users[0].user === userId, 'got the same ID back');
            })
    });
    ...
    it('should dequeue user', function (done) {
        agent.get('/api/bays/' + bayId + '/dequeue').set('Accept', 'applications/json').expect('Content-Type', /json/).expect(200).end(function (err, res) {
            expect(res.body.user).to.equal(userId);
            agent.get('/api/bays/' + bayId + '/queue').end(function (err, res) {
    ...
    it('should dequeue oldest user and newest user should still be in queue', function () {
        return agent
            ...
            .then(function (res) {
                assert(res.body.users[0].user === userId_02, 'second user still in queue');
            })
    });
    // DELETE /:bayId/queue
    it('should clear queue', function () {
            console.log(userId_01);
            return agent
                ...
                .then(function (res) {
                    assert(res.body.users.length === 2, 'queue should have 2 users');
                    return agent.delete('/api/bays/' + bayId + '/queue').expect(200)
                })
                // Make sure queue is empty
                .then(function (res) {
                    assert(res.body.users.length === 0, 'queue should be empty')
                })
        })
        // DELETE /:bayId
